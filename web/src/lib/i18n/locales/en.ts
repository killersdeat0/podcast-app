const en = {
  nav: {
    discover: 'Discover',
    queue: 'Queue',
    history: 'History',
    bookmarks: 'Bookmarks',
    playlists: 'Playlists',
    upgrade: 'Upgrade',
    profile: 'Profile',
    settings: 'Settings',
    sign_out: 'Sign out',
  },
  queue: {
    heading: 'Queue',
    empty_title: 'Your queue is empty 🎧',
    empty_description: 'Add episodes from any podcast and they\'ll line up here, ready to play.',
    empty_cta: 'Browse podcasts',
    limit_reached_guest: 'Queue limit reached — sign in and upgrade for unlimited queue 🚀',
    limit_reached_free: 'Queue limit reached — upgrade to add more episodes ✨',
    upgrade_modal_title: 'Queue limit reached 🎧',
    upgrade_modal_body: 'Free accounts can hold up to 10 episodes in the queue. Upgrade to unlock unlimited queueing.',
    upgrade_modal_cta: 'Upgrade',
    upgrade_modal_cancel: 'Maybe later',
  },
  history: {
    heading: 'History',
    empty_title: 'No listens yet 📻',
    empty_description: 'Episodes you\'ve played will show up here so you can pick up where you left off.',
    empty_cta: 'Start listening',
    filter_all: 'All',
    filter_in_progress: 'In Progress',
    in_progress_empty_title: 'Nothing in progress 🎧',
    in_progress_empty_description: 'Episodes you\'ve started but haven\'t finished will appear here.',
  },
  sidebar: {
    my_podcasts: 'My Podcasts',
    empty_hint: 'Subscribe to a podcast to see it here',
    empty_cta: 'Browse podcasts',
  },
  profile: {
    heading: 'Profile',
    account: 'Account',
    stats_section: 'Stats',
    listened: 'Hours listened',
    listened_period: 'last 30 days',
    completed_this_week: 'Completed',
    completed_period: 'this week',
    streak: 'Day streak',
    streak_week_label: '7-day streak',
    subscriptions_stat: 'Subscriptions',
    language: 'Language',
    subscriptions: 'Subscriptions',
    subscriptions_empty: 'No subscriptions yet 🔍',
    subscriptions_empty_description: 'Follow some podcasts and they\'ll appear here.',
    subscriptions_empty_cta: 'Browse podcasts',
    upgrade_cta: 'Upgrade',
    pro_label: 'Pro',
    free_label: 'Free',
    dev_downgrade: '[Dev] Downgrade to free',
    opml_export: 'Export OPML',
    opml_import: 'Import OPML',
    opml_importing: 'Importing...',
    opml_import_success: 'Imported {{n}} podcasts 🎉',
    opml_import_error: 'Import failed',
  },
  discover: {
    heading: 'Discover',
    trending: '🔥 Trending',
    continue_listening: 'Continue Listening 🎧',
    search_placeholder: 'Search podcasts...',
    search_button: 'Search',
    no_results_title: 'No results found 🔎',
    no_results_description: 'Try a different search term or browse trending podcasts.',
    loading_suggestions: 'Loading suggestions...',
    no_suggestions: 'No suggestions for',
    search_error: 'Something went wrong. Please try again.',
    results_for: 'Results for',
    featured: 'Featured',
    add_by_url_toggle: 'Have a private feed?',
    add_by_url_subtitle: 'Paste a Patreon, Substack, or any RSS feed URL to subscribe directly.',
    add_by_url_placeholder: 'Paste RSS feed URL...',
    add_by_url_fetch: 'Fetch',
    add_by_url_fetching: 'Fetching...',
    add_by_url_subscribe: 'Subscribe',
    add_by_url_subscribing: 'Subscribing...',
    add_by_url_subscribed: 'Subscribed',
    add_by_url_error_invalid_url: 'Please enter a valid feed URL.',
    add_by_url_error_fetch_failed: "Couldn't load that feed. Check the URL and try again.",
    add_by_url_error_already_subscribed: "You're already subscribed to this podcast.",
    add_by_url_error_sign_in: 'Sign in to subscribe to podcasts.',
    forYouTitle: '✨ For You',
    forYouSubtitle: 'Based on your subscriptions',
  },
  genres: {
    0: 'All',
    1303: 'Comedy',
    1318: 'Technology',
    1489: 'News',
    1488: 'True Crime',
    1321: 'Business',
    1304: 'Education',
    1324: 'Society & Culture',
    1545: 'Sports',
    1512: 'Health & Fitness',
  } as Record<number, string>,
  player: {
    playback_speed: 'Playback Speed',
    upgrade_for_speeds: 'Upgrade for more playback speeds',
    sleep_off: 'Sleep off',
    sleep_5: '5 min',
    sleep_10: '10 min',
    sleep_15: '15 min',
    sleep_30: '30 min',
    sleep_45: '45 min',
    sleep_60: '60 min',
    volume: 'Volume',
    bookmark: 'Bookmark',
    bookmark_at: (t: string) => `Bookmark at ${t}`,
    bookmark_saved: (t: string) => `Bookmark saved at ${t}`,
    bookmark_saved_hint: 'View all bookmarks →',
    bookmark_add_note: 'Add note',
    bookmark_sign_in: 'Sign in to save bookmarks',
    bookmark_error: 'Failed to save bookmark',
  },
  bookmarks: {
    heading: 'Bookmarks',
    empty: "No bookmarks yet — hit the bookmark icon in the player to save a moment",
    seek: 'Seek to',
    delete: 'Delete bookmark',
    delete_error: 'Failed to delete bookmark',
    note_placeholder: 'Add a note...',
    note_save: 'Save',
    note_cancel: 'Cancel',
  },
  welcome_toast: "Welcome back! 🎧 Great to see you again — your podcasts missed you.",
  welcome_toast_new_user: "Welcome! 🎉 Start by searching for a podcast you love.",
  guest: {
    toast_message: 'Playing as guest — sign in to save your progress across devices',
    toast_signin: 'Sign in',
    auth_prompt_title: 'Sign in to subscribe 🎙️',
    auth_prompt_body: 'Create a free account to subscribe, sync your queue, and track your listening history.',
    auth_prompt_signin: 'Sign in',
    auth_prompt_signup: 'Create account',
    auth_prompt_cancel: 'Maybe later',
    sidebar_sign_in_hint: 'Sign in to subscribe to podcasts',
    sidebar_sign_in_cta: 'Sign in →',
    queue_sync_hint: 'Sign in to sync your queue across devices and track your listening history.',
    queue_sync_cta: 'Sign in →',
    auth_prompt_history_title: 'Sign in to view your history 📖',
    auth_prompt_profile_title: 'Sign in to view your profile 👤',
    auth_prompt_upgrade_title: 'Sign in to upgrade 🚀',
  },
  podcast_page: {
    guest_nudge_title: 'Sign in to track new episodes ✨',
    guest_nudge_description: 'Get notified when new episodes drop and never lose your place.',
    guest_nudge_login: 'Sign in',
    guest_nudge_signup: 'Create account',
    search_placeholder: 'Search all episodes... 🔍',
    nav_warning_title: 'Unqueued new episodes ✨',
    nav_warning_body: 'You have {{n}} new episode(s) that will be marked as seen when you leave. Queue them to listen later.',
    nav_warning_queue_and_leave: 'Queue All & Leave',
    nav_warning_leave: 'Leave Anyway',
    nav_warning_stay: 'Stay',
    nav_warning_queuing: 'Queuing...',
    subscribe: 'Subscribe',
    subscribed: 'Subscribed ✓',
    add_to_queue: 'Add to queue',
    remove_from_queue: 'Remove from queue',
    notifications_label: 'Notifications',
    filter_off: '🔕 Off',
    filter_custom: '🎯 Custom',
    filter_help_button: 'What is this?',
    filter_help_text: 'Control which new episodes appear in your New section. All notifies you about everything. Custom narrows it to a keyword — great for podcasts that mix topics. 🔕 turns off notifications entirely.',
    filter_modal_title: '🎯 Custom episode filter',
    filter_modal_description: 'Only episodes whose title contains this keyword will appear in your ✨ New Episodes section.',
    filter_save: 'Save 🎯',
    search_results: 'Search Results',
    loading_more: 'Loading more…',
    similar_heading: 'You might also like 🎧',
    per_show_speed_label: 'Playback speed',
    per_show_speed_follow_global: 'Follow global',
    sort_newest: 'Newest',
    sort_oldest: 'Oldest',
    filter_all: 'All',
    filter_unplayed: 'Unfinished',
  },
  auth: {
    login_heading: 'Welcome back',
    signup_heading: 'Create account',
    no_account: "Don't have an account?",
    have_account: 'Already have an account?',
    sign_up_link: 'Sign up',
    log_in_link: 'Log in',
    email_placeholder: 'Email',
    password_placeholder: 'Password',
    loading: 'Loading...',
    login_button: 'Log in',
    signup_button: 'Sign up',
    or_divider: 'or',
    google_button: 'Continue with Google',
    guest_browse: 'Continue browsing as guest →',

    // Forgot password
    forgot_password_link: 'Forgot password?',
    forgot_heading: 'Reset your password 🔑',
    forgot_description: "Enter your email and we'll send you a reset link.",
    forgot_email_placeholder: 'Email',
    forgot_submit: 'Send reset link',
    forgot_success: 'Check your inbox 📬 — a reset link is on its way.',
    forgot_back_to_login: '← Back to log in',

    // Reset password
    reset_heading: 'Set new password 🔒',
    reset_description: 'Choose a strong password for your account.',
    reset_password_placeholder: 'New password',
    reset_confirm_placeholder: 'Confirm new password',
    reset_submit: 'Update password',
    reset_success: 'Password updated! Redirecting you...',
    reset_mismatch: 'Passwords do not match.',
    reset_error: 'Something went wrong. Please request a new reset link.',

    // Verify email
    verify_heading: 'Check your inbox 📬',
    verify_description: 'We sent a confirmation link to',
    verify_spam_hint: "Don't see it? Check your spam folder.",
    verify_resend: 'Resend email',
    verify_resending: 'Sending...',
    verify_resent: 'Email resent! ✓',
    verify_resend_error: 'Failed to resend. Please try again.',
    verify_back_to_login: '← Back to log in',
  },
  upgrade: {
    heading: 'Upgrade to Pro',
    free_tier_status: "You're on the free plan.",
    paid_tier_status: "You're on the paid plan. Thanks for supporting SyncPods!",
    payment_success: 'Payment successful! Your account will be upgraded shortly.',
    checkout_cancelled: 'Checkout was cancelled.',
    error_generic: 'Something went wrong.',
    error_network: 'Network error. Please try again.',
    benefits_description: 'Unlock all features: unlimited queue, full playback speed range, complete history, and no ads.',
    monthly_label: 'Monthly',
    monthly_price_suffix: '/ month',
    annual_label: 'Annual',
    annual_savings_badge: 'Save 17%',
    annual_price_suffix: '/ year',
    annual_monthly_equiv: '~$4.17/month',
    feature_unlimited_queue: 'Unlimited queue',
    feature_all_speeds: 'All playback speeds',
    feature_full_history: 'Full history',
    feature_no_ads: 'No ads',
    subscribe_monthly: 'Subscribe Monthly',
    subscribe_annually: 'Subscribe Annually',
    redirecting: 'Redirecting…',
    dev_upgrade: '[dev] upgrade to paid instantly',
    paid_management: 'You have full access to all Pro features. To manage your subscription, visit the',
    stripe_portal_link: 'Stripe customer portal',
  },
  ad_banner: {
    message: 'Enjoying SyncPods?',
    upgrade_link: 'Upgrade to Pro',
    suffix: 'for an ad-free experience.',
  },
  settings: {
    heading: 'Settings ⚙️',
    playback_section: 'Playback',
    default_speed: 'Default speed',
    default_volume: 'Default volume',
    playback_hint: 'These apply when starting a new podcast',
    skip_back: 'Default jump back',
    skip_forward: 'Default jump forward',
    skip_seconds: (n: number) => `${n}s`,
    free_speed_hint: 'Upgrade to unlock all playback speeds',
    language_section: 'Language',
    account_section: 'Account',
    sign_out: 'Sign out',
    delete_account: 'Delete account',
    delete_confirm_heading: 'Delete your account?',
    delete_confirm_body: 'This action cannot be undone. All your data will be permanently deleted.',
    delete_confirm_button: 'Yes, delete my account',
    delete_error_stripe: "Could not cancel your subscription — please contact support@syncpods.app and we'll delete your account manually.",
    delete_error_generic: 'Something went wrong. Please contact support@syncpods.app to delete your account.',
    delete_cancel: 'Cancel',
    settings_link: 'Settings →',
    about: 'About',
    contact: 'Contact',
  },
  stats: {
    heading: '📊 Your Stats',
    time_listened: 'Time listened',
    by_day_of_week: 'Listening by day',
    monthly_trend: 'Monthly trend',
    top_shows: 'Top shows',
    episodes_completed: 'episodes completed',
    no_data: 'No listening data yet — start playing some episodes! 🎧',
    free_upsell: '📊 Unlock your full listening history',
    free_upsell_description: 'You\'re seeing the last 30 days. Upgrade to Pro to see your complete all-time stats.',
    upgrade_cta: 'Upgrade to Pro',
    view_stats: 'View Stats →',
    unit_min: 'm',
    unit_hr: 'hr',
  },
  playlists: {
    heading: 'Playlists 🎙️',
    create: 'New Playlist',
    create_modal_title: 'Create Playlist',
    create_name_placeholder: 'Playlist name',
    create_description_placeholder: 'Description (optional)',
    create_submit: 'Create',
    empty_title: 'No playlists yet 🎙️',
    empty_description: 'Create a playlist to save and organize your favorite episodes.',
    empty_cta: 'Create playlist',
    play: 'Play Playlist',
    add_to_queue: 'Add to Queue',
    add_to_playlist: 'Add to Playlist',
    search_placeholder: 'Search playlists…',
    search_no_results: 'No playlists found',
    copy_link: 'Copy link',
    link_copied: 'Link copied! ✓',
    public_badge: 'Public',
    private_badge: 'Private',
    make_public: 'Make public',
    make_private: 'Make private',
    delete: 'Delete playlist',
    delete_confirm: 'Are you sure you want to delete this playlist? This cannot be undone.',
    delete_confirm_cta: 'Delete',
    remove_episode: 'Remove',
    episode_count: '{{n}} episodes',
    over_limit_playlists: 'You have more than 3 playlists. Upgrade to keep all of them active.',
    over_limit_episodes: 'This playlist has more than 10 episodes. Upgrade to keep them all.',
    upgrade_cta: 'Upgrade to Pro',
    limit_reached_playlists: 'Free plan allows up to 3 playlists. Upgrade for unlimited playlists.',
    limit_reached_episodes: 'Free plan allows up to 10 episodes per playlist. Upgrade for unlimited.',
    sidebar_heading: 'My Playlists',
    sidebar_empty_hint: 'No playlists yet',
    guest_hint: 'Sign in to create playlists',
    no_playlists_toast: 'Create a playlist first',
    no_playlists_toast_cta: 'Go to Playlists',
    auth_prompt_title: 'Sign in to use playlists 🎙️',
    auth_prompt_shared_title: 'Sign in to view this playlist 🎙️',
    auth_prompt_shared_body: 'Create a free account or sign in to view and play this playlist.',
  },
  about: {
    title: 'SyncPods 🎙️',
    tagline: 'The podcast player that keeps up with you.',
    feature_sync: 'Your progress, queue, and subscriptions sync across all your devices — pick up exactly where you left off.',
    feature_rss: 'Subscribe to any podcast via RSS, including private Patreon and Substack feeds.',
    feature_organize: 'Organize your listening with playlists and bookmarks, and discover new shows along the way.',
    feature_notifications: 'New episodes are flagged automatically — a badge on each show tells you what\'s dropped since you last visited.',
    body_3: "Built by someone with a probably-too-long podcast queue and strong opinions about skip intervals. If something feels off or you have an idea, we'd love to hear from you.",
    contact_link: 'Get in touch →',
  },
  welcome: {
    title: 'Welcome to SyncPods 🎉',
    tagline: "Here's what you can do:",
    cta: 'Start exploring →',
    guest_title: 'SyncPods 🎙️',
    guest_tagline: 'Save your progress across every device — free.',
    guest_cta: 'Create free account →',
    guest_dismiss: 'Maybe later',
  },
  contact: {
    heading: 'Contact Us 👋',
    subheading: "We're a small team and we read every message. Don't hesitate to reach out.",
    support_label: 'Support',
    support_email: 'support@syncpods.app',
    support_description: 'Account issues, billing, or anything broken',
    feedback_label: 'Feedback',
    feedback_email: 'feedback@syncpods.app',
    feedback_description: 'Feature ideas, suggestions, or just to say hi',
    response_time: 'We typically respond within 1–2 business days.',
  },
}

export default en
