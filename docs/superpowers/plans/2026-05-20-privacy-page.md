...
**Goal:** Add a public `/privacy` page to the Next.js web app that renders the Syncpods Privacy Policy, accessible at `https://syncpods.app/privacy` for Google Play and users.

**Architecture:** Single server-component page at `web/src/app/privacy/page.tsx` — outside the `(app)` and `(auth)` route groups so it inherits only the root layout (theme + fonts) with no sidebar, no auth check, and no player bar. Content is static JSX styled with existing Tailwind/MD3 color tokens.

**Tech Stack:** Next.js 16 App Router, Tailwind CSS v4, Material Design 3 color tokens via CSS custom properties.
...
- [ ] **Step 1: Create `web/src/app/privacy/page.tsx`** with the following complete content:
...
```bash
ls web/src/app/privacy/
```

Expected output: `page.tsx`

- [ ] **Step 3: Build to check for TypeScript/JSX errors**
...
Open `http://localhost:3000/privacy` in a browser. Verify:
- Page renders with dark background and correct typography
- All 14 sections are visible
- Tables render correctly with borders
- Email links are styled in the primary color
- No sidebar or player bar visible (it's outside the `(app)` layout)

- [ ] **Step 5: Commit**

```bash
git add web/src/app/privacy/page.tsx docs/privacy-policy.md
git commit -m "feat: add public /privacy page and privacy policy doc"
```
