...
### Setup
- [x] Restructure repo into monorepo (`/web`, `/mobile`, shared `/packages`)
- [x] Copy SyncPods CMP project (`/Users/personal/AndroidStudioProjects/SyncPods`) → `/mobile`
      and strip `.git` entirely: `cp -r SyncPods /mobile && rm -rf /mobile/.git`
      (also exclude `.gradle`, `build/`, `.idea/`, `.kotlin/`, `local.properties`)
- [x] Add Supabase KMP client to `/mobile` (`io.github.jan-tennerd:supabase-kt`)
- [x] Configure Supabase client via expect/actual (Android + iOS)
- [x] Update `/mobile/.env.example` — replace `EXPO_PUBLIC_` prefixes with `SYNCPODS_`
      (read via BuildConfig on Android, Info.plist on iOS)
- [x] Add Compose Multiplatform Navigation (`org.jetbrains.androidx.navigation:navigation-compose`) for screen routing
- [x] Add remaining libs to `gradle/libs.versions.toml`: Ktor, Koin, Coil, kotlinx-datetime,
...
- [x] Extract `/api/podcasts/feed` (RSS parser) → Supabase Edge Function
- [x] Extract `/api/podcasts/search` (iTunes proxy) → Supabase Edge Function
- [x] Update web app to call Edge Functions instead of its own API routes (single source of truth)
