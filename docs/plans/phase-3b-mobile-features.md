...
| Screen | Contents |
|--------|----------|
| **Discover** | "Discover" title; search bar ("Search podcasts, episodes..."); "Have a private feed? Add by URL" collapsed toggle (expands URL input → fetch preview → subscribe); genre filter chips (horizontally scrollable); **"✨ For You"** section (authenticated, ≥1 subscription — horizontal scroll row of personalized recommendations, hidden for guests/0-subs); Trending section (large 2-col artwork cards); podcast result grid (when searching); mini player bar |
...
- [ ] **"For You" recommendations** — `ForYouFeature`; new Edge Function `podcasts-recommendations` (equivalent of web's `GET /api/podcasts/recommendations`): accepts Supabase JWT, fetches user's subscriptions + `listening_by_show`, runs genre-affinity algorithm (top 2 seeds by listen time pinned + 3 random, weighted genre search with serendipity slot), returns up to 36 results; cache 2 hours in-memory keyed per user (clear on sign-out); hidden for guests and 0-subscription users
- [ ] **Similar podcasts improvements** — Podcast Detail "You might also like" section (if present) should use the same multi-pass logic as web's `/api/podcasts/similar`: same-network/producer search pass (using `artistName`), genre cap of 6, and quality filter (`trackCount < 5`); implement in a shared Edge Function or Kotlin helper
